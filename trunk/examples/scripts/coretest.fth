...
\ s" fth" environment? [if] 2drop #t [else] #f [then] value fth?
s" fth" environment? [if] drop #t [else] #f [then] value fth?
...
#f CONSTANT <FALSE>   \ modified from 0S - CWT 07.03.23
#t CONSTANT <TRUE>    \ modified from 1S - CWT 07.03.23
...
: IFFLOORED
   [ -3 2 / -2 <> ] LITERAL IF POSTPONE \ THEN ;
: IFSYM
   [ -3 2 / -1 <> ] LITERAL IF POSTPONE \ THEN ;
...
fth? not [if]
  ...
  {{ 0 0 GN' .' >NUMBER -> 0 0 GN-STRING }}
[else]
  {{ 0 GN' 0' >NUMBER -> 0 GN-CONSUMED }}
  {{ 0 GN' 1' >NUMBER -> 1 GN-CONSUMED }}
  {{ 1 GN' 1' >NUMBER -> BASE @ 1+ GN-CONSUMED }}
  {{ 0 GN' -' >NUMBER -> 0 GN-STRING }}	\ SHOULD FAIL TO CONVERT THESE
  {{ 0 GN' +' >NUMBER -> 0 GN-STRING }}
  {{ 0 GN' .' >NUMBER -> 0 GN-STRING }}
[then]
...
[else]
  {{ 0 GN' 2' 10 >NUMBER-BASED -> 2 GN-CONSUMED }}
  {{ 0 GN' 2'  2 >NUMBER-BASED -> 0 GN-STRING }}
  {{ 0 GN' F' 10 >NUMBER-BASED -> F GN-CONSUMED }}
  {{ 0 GN' G' 10 >NUMBER-BASED -> 0 GN-STRING }}
  {{ 0 GN' G' MAX-BASE >NUMBER-BASED -> 10 GN-CONSUMED }}
  {{ 0 GN' Z' MAX-BASE >NUMBER-BASED -> 23 GN-CONSUMED }}
[then]
